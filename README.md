...
**dnsacme-policy** — OpenBao plugin for ACME certificate issuance via DNS-01
challenges with role-based domain authorization
...
$ bao write dnsplugin/config/roles/<NAME> provider=<PROVIDER>
    allowed_names=<NAMES> <CREDENTIALS>
...
.Bd -literal
$ bao write dnsplugin/config/roles/<ROLE> provider=<PROVIDER>
    allowed_names=<NAMES> <CREDENTIALS>
...
.Bd -literal
Input:
  provider       DNS provider name (e.g. cloudflare, route53, gandi)
                 Any provider supported by go-acme/lego is valid.

  allowed_names  Comma-separated list of domain names or glob patterns.
                 Wildcards: *.example.com matches foo.example.com but not
                 example.com.

  <CREDENTIALS>  One or more provider-specific credential keys. Each key
                 must be a valid environment variable name for the provider.
                 Two forms are supported:

                 1. Explicit env var name (key contains an underscore):
                    e.g. CLOUDFLARE_DNS_API_TOKEN=token_value

                 2. Short key auto-prefixed with uppercase provider name:
                    e.g. api_token=token_value  →  CLOUDFLARE_API_TOKEN
.Ed
...
Input:
  ...
  name         Entity name (used for entity authorization when combined
               with metadata)
...
Output (completed):
  ...
  certificate PEM-encoded certificate bundle (leaf + intermediates)
  issued_at   Issuance timestamp
  not_after   Certificate expiry timestamp
...
Form 1: Explicit env var name (key contains an underscore)
  ...
  → lego receives: CLOUDFLARE_DNS_API_TOKEN=cfut_token

Form 2: Short key auto-prefixed with uppercase provider name
  api_token=cfut_token
  → lego receives: CLOUDFLARE_API_TOKEN=cfut_token
.Ed
...
.
config/acme_key
  ACME account (email, private key)

config/roles/<name>
  DNS provider role

enroll/<id>
  Enrollment state (CSR, domains, status, certificate, timestamps)
.Ed
